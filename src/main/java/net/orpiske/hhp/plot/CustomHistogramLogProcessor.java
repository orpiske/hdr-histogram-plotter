...
/**
 * Note f/ Otavio (temporary work-around until PR #140 is merged
 * PR 140 -> https://github.com/HdrHistogram/HdrHistogram/pull/140
 */
...
/**
 * {@link CustomHistogramLogProcessor} will process an input log and
 ...
 * <p>
 * CustomHistogramLogProcessor will process an input log file when provided with
 ...
 * (e.g. "-o mylog"), CustomHistogramLogProcessor will produce both output files
 * under the names {@code <logfile>} and {@code <logfile>.hgrm} (e.g. mylog and mylog.hgrm).
 * <p>
 * When not provided with an output file name, CustomHistogramLogProcessor will
 * produce [only] the histogram percentile distribution log output to
 * standard output.
 * <p>
 * By default, CustomHistogramLogProcessor only processes hlog file lines lines
 ...
 * <p>
 * CustomHistogramLogProcessor accepts optional -start and -end time range
 ...
 * <p>
 * CustomHistogramLogProcessor also accepts and optional -csv parameter, which
 * will cause the output formatting (of both output file forms) to use
 * a CSV file format.
 */
class CustomHistogramLogProcessor extends Thread {

    static final String versionString = "Histogram Log Processor version (custom)";
    ...
    /**
     * Construct a {@link CustomHistogramLogProcessor} with the given arguments
     ...
     */
    public CustomHistogramLogProcessor(final String[] args) throws FileNotFoundException {
        this.setName("CustomHistogramLogProcessor");
    ...
    public static void main(final String[] args)  {
        final CustomHistogramLogProcessor processor;
        try {
            processor = new CustomHistogramLogProcessor(args);
            processor.start();
        } catch (FileNotFoundException ex) {
